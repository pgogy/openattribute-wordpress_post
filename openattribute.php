...
function openattribute_firstrun() {
	if ( get_option( 'openattribute_firstrun' ) == '1' ) {
		...
		echo '<p><img src="' . esc_url( WP_PLUGIN_URL . '/' . str_replace( basename( __FILE__ ), '', plugin_basename( __FILE__ ) ) . 'information.png' ) . '" /></p>';
...
function openattribute_options_page() {
	...
	<div style="width:95%; padding:10px; border:1px solid black"><b><u>Choosing how the visitor sees the attribution</u></b><br/>
		<p>The attribution can appear in two ways (both can be used if you wish).</p>
		...
		<div style="float:left; position:relative; width:50%; text-align:center; height:75px; padding-bottom:10px;">
			<p><a href="">Attribute this resource</a></p>
			<p>As a link such as the one above.</p>
		</div>
		<br>
...
function openattribute_augment_feed( $content ) {
	if ( get_option( 'openattribute_site_license' ) != '' ) {
		if ( get_option( 'openattribute_blogoverride' ) == 0 ) {
			echo '<cc:license>' . esc_html( get_option( 'openattribute_site_license' ) ) . '</cc:license>';
			echo '<dc:rights>' . esc_html( get_option( 'openattribute_site_license' ) ) . '</dc:rights>';
		} else {
			$display = true;
			...
			if ( get_option( 'openattribute_blogoverride' ) == 1 ) {
				...
				if ( $display ) {
					...
					if ( count( $data ) != 1 ) {
						...
						echo '<creativeCommons:license>' . esc_html( $license[0] ) . '</creativeCommons:license>';
						echo '<dc:rights>' . esc_html( $license[0] ) . '</dc:rights>';
					}
				}
			}
		}
...
function openattribute_stylesheet() {
    wp_enqueue_style( 'openattribute_popup', esc_url( WP_PLUGIN_URL . '/openattribute-for-wordpress/openattribute_popup.css' ), [], false, 'screen,projector' );
}
...
add_action( 'wp_enqueue_scripts', 'openattribute_stylesheet' );
